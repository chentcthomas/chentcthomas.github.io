...
The same thing happened with our database connection. We didn’t make any special technical decisions. Prisma Client was simply taught to us from the beginning, and we used it exactly the way we learned—sharing one database connection across the whole application. It felt normal at the time, but later I learned that this approach is very close to the Singleton pattern. Even though we never intended to use any specific design pattern, some of the techniques we learned in class were already applying them without us noticing.
